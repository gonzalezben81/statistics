...
In this chapter we will discuss chi-square and correlation. Both are statistical tests that test an association or relationship between variables. These tests allow us to see if a relationship exists between nominal, ordinal, and interval variables. Certain statistical test such as the $Phi$ correlation can be used to test an association between two categorical variables. Pearson's $r$ allows us to see if there is an association between two interval variables. While Chi-square $X^2$ allows us to see is there is an association between nominal variables. There are several other correlation tests that can be run in addition to the one's mentioned above. 

---

Statistical tests in this chapter:
...
student <- read.csv(file = 'student/student.csv',sep = ",")
...
Here we want to investigate if there is an association between the type of job a mother has and a students access to the internet.
...
Next we can investigate if there is an association between the type of guardian a student has and as students access to the internet.
...
First we will want to create a contingency table for the data we want to investigate. We can do this in R simply by utilizing the $table$ function and specifying two columns of either $dichotomous$ or $binary$ variables.
